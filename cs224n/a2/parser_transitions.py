...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.buffer : list[str] = self.sentence.copy()
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            raise ValueError("transition type must be S LA or RA")
...
def minibatch_parse(sentences, model, batch_size):
    ...
    def is_empty(parser:PartialParse):
        return True if len(parser.stack) == 1 and len(parser.buffer) == 0 else False
    ...
    while len(unfinished_parses) > 0:
        ...
        for p,trans in zip(batch_parses,transitions):
            p.parse_step(trans)
            if is_empty(p):
