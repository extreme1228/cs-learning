...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        emb_siz = self.n_features * self.embed_size
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(emb_siz,self.hidden_size))
        self.embed_tohiden_bias = nn.Parameter(torch.empty(self.hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        nn.init.uniform_(self.embed_tohiden_bias)
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings,0,w.reshape(-1)) # (bz*n_features,em_siz)
        x = x.reshape(batch_size,-1) # (bz,n_features*em_siz)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        h = F.relu(x@self.embed_to_hidden_weight + self.embed_tohiden_bias)
        h = self.dropout(h)
