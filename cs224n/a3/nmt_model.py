...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        ###         https://pytorch.org/docs/stable/generated/torchnn.utils.rnn.pack_padded_sequence.html
        ...
        # Bid LSTM for encoder
        X = pack_padded_sequence(input=X,lengths=source_lengths,batch_first=False,enforce_sorted=True)
        X,(last_hidden,last_cell)= self.encoder(X)
        X,lens_unpack= pad_packed_sequence(sequence=X,batch_first=False)
        enc_hiddens = torch.permute(X,(1,0,2))
        
        # Init decoder
        bz = last_hidden.shape[1]
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # construct Y for target model embedding
        Y = self.model_embeddings.target(target_padded)
        tgt_len,bz = Y.shape[0],Y.shape[1]

        # decode in time t
        o_t = torch.zeros(bz,self.hidden_size)
        # print(Y.shape)
        for y_t in torch.split(Y,1,dim=0):
            y_t = torch.squeeze(y_t,dim=0)
            # print(y_t.shape)
            ybar_t = torch.cat([y_t,o_t],dim=1)
