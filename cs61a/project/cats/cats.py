...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words),len(source_words))):
        if typed_words[i] == source_words[i]:
            correct_words += 1
    return correct_words / all_words * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    # match_word = typed_word
    # diff = -1
    # for word in word_list:
    #     now_diff = diff_function(typed_word,word,limit)
    #     if now_diff <= limit:
    #         if diff == -1:
    #             diff = now_diff
    #             match_word = word
    ...
    words_diff = [diff_function(typed_word,word,limit) for word in word_list]
    match_word ,diff = min(zip(word_list,words_diff),key=lambda x:x[1])
    if diff <= limit:
        return match_word
    else:
        return typed_word
